...
public final class DlgResepPulang extends javax.swing.JDialog {
    ...
    public DlgResepPulang(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[] {
                "No.Rawat", "Tgl.Resep", "Jam", "Pasien", "Obat", "Jml", "Harga", "Total", "Dosis", "No.Batch",
                "No.Faktur", "Gudang" }) {
            @Override
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
                JOptionPane.showMessageDialog(rootPane,"Data billing sudah t erverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
                 TCari.requestFoc us () ;
            else{ 
               if(tbResep.getSelectedRow()!= -1){
                   ...
                   sukses=true;
                     ...
                     if(Sequel.queryu2tf("delete from  r es ep_pulang where no_rawat=? and kode_brng=? and no_batch=? and no_faktur=? and tanggal=? and jam=?",6,new String[]{
                        tbResep.getValueAt(i,0).toStrin
                        g(),KdBarang.getText(),tbResep.getValueAt(i,9).toString(),tbResep.getValueAt(i,10).toString(),tbResep.getValueAt(i,1).toString(),tbResep.getValueAt(i,2).toString()
                    })==true){
                          if(aktifkanbatch.equals("yes")){
                             Sequel.mengedit 3("dat a_batch","no_batch=? and kode_brng=? and no_faktur=?","sisa=sisa+?",4,new String[]{
                             ...
                             Sequel.menyi
                            mpan("gudangbarang","'"+KdBarang.getText()+"','"+tbResep.getValueAt(i,11).toString()+"','"+tbResep.getValueAt(i,5).
                            to String()+"', '
                                                     "sto k=stok+'"+tbRe sep.getValueAt(i,5)
                                    .toString()+"'","kode _brng='"+KdBar ang.getText()+"' and  kd_bangsal='"+t
                                    bResep.getValueAt(i,1 1).toString()+ "' and no_batch='"+tb Resep.getValu
                                  }else {
                             Trackobat.catatRiwayat(KdBa rang.getText(),Valid.SetAngka(tbResep.getValueAt(i,5).toString()),0,"Resep Pulang",akses.getkode(),tbResep.getValueAt(i,11).toString(),"Hapus","","",TNoRw.getText()+" "+TPasien.getText());
                            Sequel.menyimpan("gudangbarang ","'"+KdBarang.getText()+"','"+tbResep.getVal ueAt(i,11).toS
                                    tr ing()+"','"+ t
                                                       "stok=stok+'"+t bResep.getValu
                                            eAt(i,5).toString()+" '","kode_brng= '"+KdBarang.getText
                                            ()+"' and kd_bangsal= '"+tbResep.get
    ...
    private void  BtnH apusKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnHapusKeyPressed
    ...
    private void BtnKeluarKeyPressed(j ava.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnKeluarKeyPressed
        i
                 dispose();
        ...
        if(tabMode.getRowCount()==0){ 
             JOptionPane.sho wM essageDialog(null, "Maaf, data sudah habis. Tidak ada data yang bisa anda print...!!!!");
            TCari.requestFocus();
        } else  
            if(tabMode.getRow Count()!= 0){
        
                 Sequel.queryu("delete from temporary where temp37='"+akses.getalamatip()+"'");
            
            for(int i=0;i<tabMode.getRowCount();i++){   
                Sequel.menyimpan("temporary","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                     ""+i,tabMode .g et ValueAt(i,0).toString(),tabMode.getValueAt(i,1).toString(),tabMode.getValueAt(i,2).toString(),
                    tabMode.getValueAt(i,3).toS tring(),tabMode.getValueAt(i,4).toString(),tabMode.getValueAt(i,5).toString(),
                    ...
                    "","","","","","","","","","","","","","","" , akses.getalamatip()  
                                ...
                                ng, Object> param = n ew HashMap<>() ;   
                                 
                                m.put("namars",akses.getnamars()); 
                                 
                                m.put("alamatrs",akse s.getalamatrs( )); 
                                           
                                m.p ut( "ko tar s", aks es. get kab upa ten rs( ));   
                        param.put("propinsirs",akses.getpropinsirs());
             

            Valid.MyReportqry("rptResepPulang.jasper","r
             
            Cursor(Cursor.getDefa ultCursor());
            t_BtnPrintActionPer formed
             
             BtnPrintKeyPressed(j ava.awt.event.KeyEven
            etKeyCode()==KeyEven t.VK_SPACE){
            rintActionPerform ed(null);
            e{  
    ...
    private void TCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN -FIRST:event_TCariKeyPressed
         i f(evt.getKeyCode( )= =KeyEvent.VK_ENTER ){
    ...
    private void BtnAllActionPerformed(java.awt.event.ActionEvent e vt) {//GEN-FIRST:event_BtnAllActionPerformed
        TC ari.setText("");   
        runBackground(() ->tampil());        
}//GEN-LA ST:e vent_BtnAllActionPerformed
    ...
    priva
            ...
            Valid.pindah(ev t, BtnCari
             }
}//GEN-LAST:event_BtnAllKeyPressed
 
    privat e void tbResepMou se Clicked(java.awt.e vent.MouseEvent evt) {//GEN-FIRST:event_tbResepMouseClicked
        if(tabMode.getRowCount( )!=0){
            try {
                  getData();
    ...
    privat e void tbResepKeyPress ed (j ava.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbResepKeyPressed
        if(tabMode.getRowCount()!=0){
...
}//GEN-LAS T:event_tbResepKeyPres se d 
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
    ...
    private void btnBarangKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_btnBarangKeyPressed
        V
         }//GEN-LAST:event_btnBarangKeyP

        private void btnBarangActionPerformed(java.awt.event.ActionEvent evt ) {//GEN-FIRST:event_btnBarangActionPer
              if(
                     JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
                TCari.requestFocus();
        }else{ 
            bangsal=Seque l.cariIsi (
                ...
                "where kamar_inap.no_rawat=? and kamar_inap.stts_pulang='-' o rder by STR_TO_DATE(concat(kamar_inap.tgl_masuk,' ',jam_masuk),'%Y-%m-%d %H:%i:%s') desc limit 1",TNoRw.getText());
             lokasistok=Sequel.cariIsi("select set _ de p
            okasistok.equals("")){
            ...
            if(lokasidepoutama.getDepoDefault().equals("")){
                    ...
                    lo k asistok=lokasid
                    epoutama.getDepoDefault();
                     
                 } 
            }  
            akses.setkdbangsal(lokasistok);
            DlgInputResepPulang inputresep=new DlgInputResepPulang(null,false);
            inputr esep.addWindowListener(new WindowListener() { 
                @Override  
                p ubli c void windowOpened(WindowEvent e) {}
                @Override  
                ...
                public void windowClosed(WindowEvent e) {
                    if(inputresep.getTable ( ).getSelectedRow()!= -1){                    
                        ...
                        Satuan.setText(inputresep.getTabl
                e
                ...
                public void windowDeico
                                nified(WindowEvent e) {} 
                @Override 
                publi
                    rride
                p
            ...
            inputresep.setNoRm(TNoRw.getText(),TNoRM.getText
                (

                tresep.tampil();
            inputresep.setSize(internalFrame1.getWidth()-20,in
                t

                tresep.setLocationRelativeTo(internalFrame1);
    ...
    private void TNoRwKeyPressed(java.awt.event .KeyEvent evt) { //GEN-FIRST:event_ TNoR
                    wKeyPressed 
        if(evt.getKeyCode()==KeyEvent.VK_PAGE_DOWN){
            Sequel.cariIsi("select reg_periksa.no_rkm_me d is  from reg_periksa where reg _ periksa.no_rawat=? ",TNoRM,TNoRw.getText());
    ...
    }//GEN -LAST:event_TNoRw Ke yPressed 
 
                    
    private void ppResepObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST :event_p pResepObatActionPerformed
        i f(tabMo de.getRowCount()= =0 ){ 
            JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
             T Cari.requestFocus();
        }else if(tbResep.getS electe dRow()<= -1){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan pilih data..!!");
         }else{
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            Map<String, Object> param = new HashMap<>();   
             param.put("namars",a ks es .getnamars());
            param.put("alamatrs",akses.getalama trs());
            param.put("kotars",akses.getkabupatenrs());
             para m.put("propinsirs",akses. getpro pinsirs());
            param.put("kontakrs",akses.getkonta krs());
             p aram.put("emailrs",akses.getemailrs());
            if(Sequel.cariInteger(
                    "select count(*) from resep_pulang w
                param.put("logo ",Sequel.cariGambar("select setting.logo from setting")); 
                Valid.MyReportqry ("rptItemResepPulang.jasper","report","::[ Aturan Pakai Obat ]::",
                    "select res ep_pulang.no_rawat,resep_pulang.tanggal, "+
                    "reg_periksa.no _rkm_medis,pasien.nm_pasien,databarang.nama_brng,"+
                    "resep_pulang .dosis,resep_pulang.jml_barang,kodesatuan.satuan "+
                    "from resep_ pulang inner join reg_periksa on resep_pulang.no_rawat=reg_periksa.no_rawat "+
                     "inner join databarang on resep_pulang.kode_brng=databarang.kode_brng "+
                    "inner join kodesatuan on databarang.kode_sat=kodesatuan.kode_sat "+
                       
                    "inner join p asien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    "where resep_pulang.no_rawat='"+TNoRw.getT ext()+"'  and resep_pulang.dosis<>''",
                                ...
                                esepObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIR S
                                etRowCount()==0){ 
                                ane.showMessageDialog(null,"Maa f , data sudah ha b is...!!!!");
                        
            T
            e if(tbResep.getSelectedRow()<= -1){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan pilih data..!!");
         }else{
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            Map<String, Object> param = new HashMap<>(); 
            ...
            if(Sequel.cariInteger(
                ...
                Valid.MyReportqry ("rptItemResepPulang2.jasper","report","::[ Aturan Pakai Obat ]::",
                    "select res ep_pulang.no_rawat,resep_pulang.tanggal,jenis.nama,"+
                    "reg_periksa.no _rkm_medis,pasien.nm_pasien,databarang.nama_brng,"+
                    "resep_pulang .dosis,resep_pulang.jml_barang,kodesatuan.satuan "+
                    "from resep_ pulang inner join reg_periksa on resep_pulang.no_rawat=reg_periksa.no_rawat "+
                     "inner join databarang on resep_pulang.kode_brng=databarang.kode_brng "+
                    ...
                    "inner join j enis on databarang.kdjns= jenis.kdjns "+
                    "inner join pasien on reg_periksa.no_rkm_me dis=pasie n.no_rkm_medis "+
                        "where resep_pulang.no_rawat='"+TNoRw.getText()+"' and resep_pu l
                                ...
                                esepObat2ActionPerformed(java.awt.event.ActionEvent  e
                                etRowCount()==0){ 
                                ane.showMessageDialog(null,"Maa f , data sudah ha b is...!!!!");
            ...
            if(Sequel.cariInteger(
                ...
                Valid.MyReportqry ("rptItemResepPulang3.jasper","report","::[ Aturan Pakai Obat ]::",
                    ...
                    "inner join p asien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    "where resep_pulang.no_rawat='"+TNoRw.getTe xt()+"' a nd resep_pulang.dosis<>''",p
                                ...
                                mWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_fo r
                                .CARICEPAT().equals("aktif")){ 
                                tDocument().addDocumentListener ( new javax.swing . event.DocumentListener(){
                ...
                @Override 
               public v o id remove U pdate(DocumentEvent e) {
                    if(TCari.getText().length()>2){
                        runBackground(() ->tampil());
                    }
                }
        ...
        if (tbResep.getSelectedRow() != -1) {
            ...
            for (int i = 0; i < selectedRows.length; i++) {
                ...
                Object cellValue = tbResep.getValueAt(rowIndex, 4);
                    ...
                    sb.appe n d(cellVa l ue.toString().split("\\s+")[1].trim());
                }    
                Object cellValue2  =  tbResep.getVal u eAt ( rowIndex, 4);  
    ...
    */
    ...
    private void tampil() {
        ...
        try{      
            ...
            try {
                  ...
                  ps.setString(2,Valid.Set
                    Tgl(DTPCari2.getSelectedItem()+""));
                            ...
                            ps.setString(6,"%"+TCari.getText() . tr
                                    i m()+"%"); 
                ...
                while(rs.next()){     
                    tabMode.addRow( new   Object[]{  
                        rs.getStrin g(1 ) ,rs.getString(2),rs.ge t String(3),rs.getString(4),rs.getString(5),rs.getDouble(6),rs.getDouble(7),rs.getDouble(8),rs.getString(9),rs.getString(10),rs.getString(11),rs.getString(12)
                    });     
                    jumlahtotal=jum lah t otal+rs.getDouble("tot a l");
                }     
            } cat
            ...
            System .ou t. print ln("Notifikasi : "+e);
        }        
    }
    ...
    private void getData() {  
        i f(tbR esep.getSelected Row()!= -1){
            TNoRw.setText(tbResep.getValueAt(t b Resep.getSelectedRow(),0).toString()); 
    ...
    public void setNoRm(String norwt,String norm,String pasien,Date tgl1,Date tgl2) {
        TN oRw.setText(norwt);  
        TNoRM.setText(norm); 
            ien.setText(pasien); 
            el.cariIsi("sel
                    ect concat(pasien.no_rkm_medis,' ',pasien.nm_pasien) from reg_periksa inner join pasien "+
                      
                    " on re
                    g_periksa.no_rkm_medis=pasien.no_rkm_medis where reg_periksa.no_rawat=? ",TPasien,TNoRw.getText());
                      
        TCari.setText(norwt
                    );     
                      
        TPCari1.setDate(tgl1);
    ...
    private void isForm(){
        if(ChkInput.isSelected()==true){ 
            ...
            ChkInput.setV isible(true);
        }    
    }
     
    public void isCek(){
            arang.setEnabled(akses.getresep_pulang());
        B tnHapus .setEnabled(akses.getr es ep_pul a
            rint.setEnabled(akses.getre
