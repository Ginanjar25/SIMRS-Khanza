...
public class DlgPiutang extends javax.swing.JDialog {
    ...
    private double ttljual = 0, stok, jumlah, ppnobat = 0, besarppnobat = 0, tagihanppn = 0, ongkir = 0, uangmuka = 0,
            sisapiutang = 0;
    private PreparedStatement ps;
    private ResultSet rs;
    private String aktifkanbatch = "no", pilihanetiket, hppfarmasi = "", DEPOAKTIFOBAT = "", DEPOAKTIFOBAT = "",
            kd_bangsal = "";
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Transaksi Piutang Obat, Alkes & BHP Medis ]::",
        ...
        Jenisjual.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Jual Bebas", "Harga Beli", "Karyawan",
                "Beli Luar", "Rawat Jalan", "Kelas 1", "Kelas 2", "Kelas 3", "Utama", "VIP", "VVIP" }));
        ...
        if (aktifkanbatch.equals("yes") && NoFaktur.getText().equals("")) {
            ...
            try {
                ...
                if (jumlah > stok) {
                    ...
                    JOptionPane.showMessageDialog(null, "Maaf, stok tidak mencukupi. Maksimal yang bisa diretur " + stok
                            + " " + satuanbar.getText());
                } else {
                    if (Sequel.menyimpantf("tamppiutang",
                            "'" + kdbar.getText() + "','" + nmbar.getText() + "','" + satuanbar.getText() + "','"
                                    + HrgJual.getText() + "','" + HrgBeli.getText() + "','" + Jmljual.getText() + "','"
                                    + subtotal.getText() + "','" + Disc.getText() + "','" + Bsrdisc.getText() + "','"
                                    + Total.getText() + "','" + NoBatch.getText() + "','" + akses.getkode() + "','"
                                    + NoFaktur.getText() + "','" + Aturan.getText() + "'",
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnHapusActionPerformed
        if (nmbar.getText().trim().equals("")) {
            ...
            Valid.hapusTable(tabMode, kdbar, "tamppiutang", "no_batch='" + NoBatch.getText() + "' and no_faktur='"
                    + NoFaktur.getText() + "' and petugas='" + akses.getkode() + "' and kode_brng");
            runBackground(() -> tampil());
            emptTeks();
            isSisaPiutang();
        }
            ...
            member.addWindowListener(new WindowAdapter() {
                ...
                public void windowClosed(WindowEvent e) {
                    if (member.getTable().getSelectedRow() != -1) {
                        kdpasien.setText(
                                member.getTable().getValueAt(member.getTable().getSelectedRow(), 0).toString());
                        nmpasien.setText(
                                member.getTable().getValueAt(member.getTable().getSelectedRow(), 1).toString());
                    }
                ...
                barang.addWindowListener(new WindowAdapter() {
                    ...
                    public void windowClosed(WindowEvent e) {
                        if (barang.getTable().getSelectedRow() != -1) {
                            kdbar.setText(
                                    barang.getTable().getValueAt(barang.getTable().getSelectedRow(), 1).toString());
                            nmbar.setText(
                                    barang.getTable().getValueAt(barang.getTable().getSelectedRow(), 2).toString());
                            satuanbar.setText(
                                    barang.getTable().getValueAt(barang.getTable().getSelectedRow(), 6).toString());
                            if (hppfarmasi.equals("dasar")) {
                                HrgBeli.setText(barang.getTable().getValueAt(barang.getTable().getSelectedRow(), 10)
                                        .toString());
                            } else {
                                HrgBeli.setText(barang.getTable().getValueAt(barang.getTable().getSelectedRow(), 11)
                                        .toString());
                            }
                            ...
                            if (Jenisjual.getSelectedItem().equals("Jual Bebas")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 20).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Karyawan")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 21).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Beli Luar")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 19).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Rawat Jalan")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 12).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Kelas 1")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 13).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Kelas 2")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 14).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Kelas 3")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 15).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Utama")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 16).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("VIP")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 17).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("VVIP")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 18).toString()),
                                                100) + "");
                            } else if (Jenisjual.getSelectedItem().equals("Harga Beli")) {
                                HrgJual.setText(
                                        Valid.roundUp(
                                                Double.parseDouble(barang.getTable()
                                                        .getValueAt(barang.getTable().getSelectedRow(), 11).toString()),
                                                100) + "");
                            }

                            if (aktifkanbatch.equals("yes")) {
                                NoBatch.setText(barang.getTable().getValueAt(barang.getTable().getSelectedRow(), 32)
                                        .toString());
                                NoFaktur.setText(barang.getTable().getValueAt(barang.getTable().getSelectedRow(), 33)
                                        .toString());
                                Stok.setText(Double.toString(Sequel.cariIsiAngka(
                                        "select gudangbarang.stok from gudangbarang where gudangbarang.kd_bangsal='"
                                                + kdgudang.getText() + "' and gudangbarang.kode_brng='"
                                                + kdbar.getText() + "' and gudangbarang.no_batch='" + NoBatch.getText()
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if (NoNota.getText().trim().equals("")) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses = true;
                if (Sequel.menyimpantf2("piutang", "'" + NoNota.getText() + "','"
                        + Valid.SetTgl(TglJual.getSelectedItem() + "") + "','" + kdptg.getText() + "','"
                        + kdpasien.getText() +
                        "','" + nmpasien.getText() + "','" + catatan.getText() + "','" + Jenisjual.getSelectedItem()
                        + "','" + besarppnobat + "','" + ongkir + "','" + uangmuka + "','" + sisapiutang +
                        "','Umum','" + Valid.SetTgl(TglTempo.getSelectedItem() + "") + "','" + kdgudang.getText() + "'",
                        "No.Nota") == true) {
                    simpan();
                } else {
                    autoNomor();
                    if (Sequel.menyimpantf2("piutang",
                            "'" + NoNota.getText() + "','" + Valid.SetTgl(TglJual.getSelectedItem() + "") + "','"
                                    + kdptg.getText() + "','" + kdpasien.getText() +
                                    "','" + nmpasien.getText() + "','" + catatan.getText() + "','"
                                    + Jenisjual.getSelectedItem() + "','" + besarppnobat + "','" + ongkir + "','"
                                    + uangmuka + "','" + sisapiutang +
                                    "','Umum','" + Valid.SetTgl(TglTempo.getSelectedItem() + "") + "','"
                                    + kdgudang.getText() + "'",
                            "No.Nota") == true) {
                        simpan();
                    } else {
                ...
                if (sukses == true) {
                    ...
                    if (Sequel.menyimpantf2("tampjurnal",
                            "'" + Sequel.cariIsi("select set_akun.Piutang_Obat from set_akun") + "','PIUTANG OBAT','"
                                    + (sisapiutang) + "','0'",
                            "Rekening") == false) {
                        sukses = false;
                    }
                    if (Sequel.menyimpantf2("tampjurnal",
                            "'" + Sequel.cariIsi("select set_akun.Kontra_Piutang_Obat from set_akun")
                                    + "','PERSEDIAAN','0','" + (sisapiutang) + "'",
                            "Rekening") == false) {
                        sukses = false;
                    }
                    if (sukses == true) {
                        sukses = jur.simpanJurnal(NoNota.getText(), "U",
                                "PIUTANG DI " + nmgudang.getText().toUpperCase() + ", OLEH " + akses.getkode());
                    }
                }
                if (sukses == true) {
                    Sequel.Commit();
                                ...
                                if (Sequel.cariInteger(
                                    ...
                                    Valid.MyReportqry("rptItemResepPiutang.jasper", "report",
                                            ...
                                            "select piutang.nota_piutang,piutang.tgl_piutang, " +
                                                    ...
                                                    +
                                                    "where piutang.nota_piutang='" + NoNota.getText()
                                                    + "' and detailpiutang.aturan_pakai<>''",
                                            param);
                                }
    ...
    private void BtnNotaActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnNotaActionPerformed
        ...
        if (NoNota.getText().trim().equals("")) {
            ...
            Valid.panggilUrl("billing/NotaApotek3.php?nonota=" + NoNota.getText() + "&kdptg=" + kdptg.getText()
                    + "&muka=" + uangmuka + "&ongkir=" + ongkir + "&besarppnobat=" + besarppnobat + "&tagihanppn="
                    + tagihanppn + "&sisapiutang=" + sisapiutang + "&tanggal="
                    + Valid.SetTgl(TglJual.getSelectedItem() + "") + "&nm_member="
                    + nmpasien.getText().replaceAll(" ", "_") + "&tgltempo="
                    + Valid.SetTgl(TglTempo.getSelectedItem() + "") + "&catatan="
                    + catatan.getText().replaceAll(" ", "_") + "&usere=" + koneksiDB.USERHYBRIDWEB() + "&passwordte="
            ...
            aturan_pakai.addWindowListener(new WindowAdapter() {
                ...
                public void windowClosed(WindowEvent e) {
                    if (aturan_pakai.getTable().getSelectedRow() != -1) {
                        Aturan.setText(aturan_pakai.getTable().getValueAt(aturan_pakai.getTable().getSelectedRow(), 0)
                                .toString());
                    }
                    aturan_pakai = null;
                }
    ...
    private void kdpasienKeyPressed(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_kdpasienKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
            ...
            Sequel.cariIsi("select pasien.nm_pasien from pasien where pasien.no_rkm_medis=?", nmpasien,
                    kdpasien.getText());
    ...
    private void isDiskon() {
        if (!Disc.getText().trim().equals("")) {
            try {
                Bsrdisc.setText(Valid.SetAngka2(
                        (Double.parseDouble(Disc.getText()) / 100) * Double.parseDouble(subtotal.getText())));
            } catch (Exception e) {
                Disc.setText("0");
                Bsrdisc.setText("0");
                if (e.toString().contains("java.lang.NumberFormatException")) {
