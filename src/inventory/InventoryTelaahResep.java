...
public final class InventoryTelaahResep extends javax.swing.JDialog {
    ...
    public InventoryTelaahResep(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[] {
                "No.Resep", "Tanggal", "Jam", "No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "J.K.", "Tgl.Lahir",
                ...
                "Ket T.R.Ada/Tidak Duplikasi",
                "T.R.Interaksi Obat", "Ket T.R.Interaksi Obat", "T.R. Alergi Obat", "Ket T.R.Alergi Obat",
                "T.R. Kesesuaian Formularium", "Ket T.R.Kesesuaian Formularium", "T.O.Tepat Pasien",
                "T.O.Tepat Obat", "T.O.Tepat Dosis", "T.O.Tepat Waktu & Frekuensi", "T.O.Kesesuaian Formularium", "NIP",
                "Petugas Farmasi"
        }) {
            @Override
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Data Telaah Resep & Obat ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoResep.getText().trim().equals("") || TNoRw.getText().trim().equals("")
                || TPasien.getText().trim().equals("")) {
            Valid.textKosong(TNoRw, "resep/pasien");
        } else if (Nip.getText().trim().equals("") || NamaPetugas.getText().trim().equals("")) {
            Valid.textKosong(Nip, "Petugas");
        } else {
            if (Sequel.menyimpantf("telaah_farmasi", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 25,
                    new String[] {
                            NoResep.getText(), ResepTepatIdentifikasiPasien.getSelectedItem().toString(),
                            KetResepTepatIdetifikasiPasien.getText(),
                            ResepTepatObat.getSelectedItem().toString(), KetResepTepatObat.getText(),
                            ResepTepatDosis.getSelectedItem().toString(),
                            ...
                            ResepKontraIndikasiObat.getSelectedItem().toString(), KetResepKontraIndikasiObat.getText(),
                            ObatTepatPasien.getSelectedItem().toString(),
                            ...
                            ObatKesesuaianFormula.getSelectedItem().toString(), Nip.getText(),
                            ResepFormula.getSelectedItem().toString(), KetResepFormula.getText()
                    }) == true) {
            ...
            if (tabMode.getRowCount() == 0) {
                ...
                Valid.MyReportqry("rptDataTelaahResep.jasper", "report", "::[ Data Telaah Resep ]::",
                        "select telaah_farmasi.no_resep,resep_obat.tgl_perawatan,resep_obat.jam,resep_obat.no_rawat,reg_periksa.no_rkm_medis,"
                                ...
                                "where resep_obat.tgl_perawatan between '"
                                + Valid.SetTgl(DTPCari1.getSelectedItem() + "") + "' and '"
                                + Valid.SetTgl(DTPCari2.getSelectedItem() + "") + "' " +
                                (TCari.getText().equals("") ? ""
                                        : "and (telaah_farmasi.no_resep like '%" + TCari.getText().trim()
                                                + "%' or resep_obat.no_rawat like '%" + TCari.getText().trim()
                                                + "%' or " +
                                                "reg_periksa.no_rkm_medis like '%" + TCari.getText().trim()
                                                + "%' or pasien.nm_pasien like '%" + TCari.getText().trim()
                                                + "%' or resep_obat.kd_dokter like '%" + TCari.getText().trim()
                                                + "%' or " +
                                                "dokter.nm_dokter like '%" + TCari.getText().trim()
                                                + "%' or resep_obat.status like '%" + TCari.getText().trim()
                                                + "%' or telaah_farmasi.nip like '%" + TCari.getText().trim() + "%' or "
                                                +
                                                "petugas.nama like '%" + TCari.getText().trim() + "%') ")
            ...
            try {
                ...
                bw.write(LoadHTML.getText().replaceAll("<head>",
                        "<head><link href=\"file2.css\" rel=\"stylesheet\" type=\"text/css\" />" +
                                ...
                                "<font size='2' face='Tahoma'>DATA VERIFIKASI RESEP<br>PERIODE "
                                + DTPCari1.getSelectedItem() + " s.d. " + DTPCari2.getSelectedItem() + "<br><br></font>"
                                +
                                "</td>" +
                                "</tr>" +
                                "</table>"));
    ...
    private void btnValidasiActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnValidasiActionPerformed
        if (akses.getberi_obat() == true) {
            if (TNoRw.equals("")) {
                JOptionPane.showMessageDialog(null,
                        "Maaf, Silahkan pilih data resep yg sudah ditelaahyang mau divalidasi..!!");
            } else {
                if ("Yes".equals(cekValidasi(NoResep.getText()))) {
                    ...
                    JOptionPane.showMessageDialog(rootPane,
                            "Resep belum ditelaah, silahkan lakukan telaah resep terlebih dahulu ..!!");
                } else {
                    int jmlparsial = 0;
                    if (jmlparsial > 0) {
                        Sequel.queryu("delete from antriapotek2");
                        Sequel.queryu("insert into antriapotek2 values('" + NoResep.getText() + "','1','"
                                + TNoRw.getText() + "')");
                        panggilform();
                    } else {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[] {
                            ...
                            rs.getDate("tgl_lahir"), rs.getString("kd_dokter"),
                            rs.getString("nm_dokter"), rs.getString("status").replace("r", "R"),
                            rs.getString("resep_identifikasi_pasien"), rs.getString("resep_ket_identifikasi_pasien"),
                            rs.getString("resep_tepat_obat"), rs.getString("resep_ket_tepat_obat"),
                            rs.getString("resep_tepat_dosis"), rs.getString("resep_ket_tepat_dosis"),
                            rs.getString("resep_tepat_cara_pemberian"), rs.getString("resep_ket_tepat_cara_pemberian"),
                            rs.getString("resep_tepat_waktu_pemberian"),
                            rs.getString("resep_ket_tepat_waktu_pemberian"),
                            rs.getString("resep_ada_tidak_duplikasi_obat"),
                            rs.getString("resep_ket_ada_tidak_duplikasi_obat"),
                            rs.getString("resep_interaksi_obat"), rs.getString("resep_ket_interaksi_obat"),
                            rs.getString("resep_kontra_indikasi_obat"),
                            rs.getString("resep_ket_kontra_indikasi_obat"),
                            rs.getString("resep_kesesuaian_formularium"),
                            rs.getString("resep_ket_kesesuaian_formularium"),
                            rs.getString("obat_tepat_pasien"), rs.getString("obat_tepat_obat"),
                            rs.getString("obat_tepat_dosis"),
                            rs.getString("obat_tepat_waktu_pemberian"), rs.getString("obat_kesesuaian_formularium"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("telaah_farmasi", "no_resep=?",
                "no_resep=?,resep_identifikasi_pasien=?,resep_ket_identifikasi_pasien=?,resep_tepat_obat=?,resep_ket_tepat_obat=?,"
                        ...
                        +
                        "resep_ket_kontra_indikasi_obat=?,obat_tepat_pasien=?,obat_tepat_obat=?,obat_tepat_dosis=?,obat_tepat_waktu_pemberian=?,obat_kesesuaian_formularium=?,nip=?,resep_kesesuaian_formularium=?, resep_ket_kesesuaian_formularium =? ",
                26, new String[] {
                        NoResep.getText(), ResepTepatIdentifikasiPasien.getSelectedItem().toString(),
                        KetResepTepatIdetifikasiPasien.getText(),
                        ResepTepatObat.getSelectedItem().toString(), KetResepTepatObat.getText(),
                        ...
                        KetResepTepatDosis.getText(), ResepTepatCaraPemberian.getSelectedItem().toString(),
                        KetResepTepatCaraPemberian.getText(),
                        ResepTepatWaktuPemberian.getSelectedItem().toString(), KetResepTepatWaktuPemberian.getText(),
            ...
            ps = koneksi.prepareStatement(
                    "select resep_obat.no_resep,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,resep_obat.kd_dokter,dokter.nm_dokter,resep_obat.status,resep_obat.tgl_perawatan,resep_obat.jam "
                            ...
                            + (TCari.getText().trim().equals("") ? ""
                                    : "and (resep_obat.no_resep like ? or " +
                                            "resep_obat.no_rawat like ? or pasien.no_rkm_medis like ? or pasien.nm_pasien like ? or resep_obat.kd_dokter like ? or dokter.nm_dokter like ? or resep_obat.status like ?) ")
                                ...
                                while (rs3.next()) {
                                    htmlContent.append(
                                            "<tr class='isi'>" +
                                                    ...
                                                    "<td align='center'>&nbsp;&nbsp;" + rs3.getString("kode_sat")
                                                    + "</td>" +
                                                    "<td align='left'></td>" +
                                                    "<td align='left'>" + "&nbsp;&nbsp;" + rs3.getString("kode_brng")
                                                    + "</td>" +
                                                    "<td align='left'>" + "&nbsp;&nbsp;" + rs3.getString("nama_brng")
                                                    + "</td>" +
                                                    "</tr>");
                                }
                            } catch (Exception e) {
                                    ...
                                    "<tr class='isi'>" +
                                            ...
                                            Sequel.cariIsi(
                                                    "select aturan_pakai.aturan from aturan_pakai where aturan_pakai.tgl_perawatan='"
                                                            + rs.getString("tgl_perawatan") + "' and " +
                                                            "aturan_pakai.jam='" + rs.getString("jam")
                                                            + "' and aturan_pakai.no_rawat='" + rs.getString("no_rawat")
                                                            + "' and aturan_pakai.kode_brng='"
    ...
    private String cekValidasi(String no_resep) {
        String hasil = "";
        try {
            String angka = Sequel.cariIsi(
                    "select * from resep_obat where no_resep = ? and tgl_perawatan <> '0000-00-00' and jam <> '00:00:00'",
                    no_resep);
