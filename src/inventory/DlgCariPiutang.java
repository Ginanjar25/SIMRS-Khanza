...
public class DlgCariPiutang extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Cari Piutang Obat, Alkes & BHP Medis ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
            ...
            if (ceksukses == false) {
                ...
                for (int i = 0; i < row; i++) {
                    Sequel.menyimpan("temporary", "'" + i + "','" +
                            ...
                            + "','','','','','','','','','','','','','','','','','','','','','','','','','','"
                            + akses.getalamatip() + "'", "Transaksi Piutang");
                }
                i++;
                Sequel.menyimpan("temporary", "'" + i
                        + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','"
                ...
                Valid.MyReportqry("rptPiutang.jasper", "report", "::[ Transaksi Piutang Barang ]::",
    ...
    private void ppCetakNotaActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppCetakNotaActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().equals("")) {
                ...
                Valid.panggilUrl(
                        "billing/NotaApotek4.php?nonota=" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString()
...
//           }      
    ...
    private void ppResepObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppResepObat1ActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim().equals("")){
                ...
                if(Sequel.cariInteger(
                        "select count(*) from detailpiutang where nota_piutang=? an d aturan_pakai<>''",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim())>0){
                     Valid.MyRepo rt qr y("rptItemResepPiutang2.jasper","report","::[ Aturan Pakai Obat ]::",
                        "select piutang.nota_pi utang,piutang.tgl_piutang, "+
                        "piutang.no_rkm_medis,piutang.nm_pasien,databarang.nama_brng,"+
                          "detailpiutang.atu ran_pa kai,detailpiutang.jumlah,kodesatuan.satuan,jenis.nama as jenis "+
                        "from piutang inner joi n detailpiutang on piutang.nota_piutang=detailpiutang.nota_piutang "+
                          "inner join databarang on detailpiutang.kode_brng=databarang.kode_brng "+
                         "inner join kodesatuan on databarang. kode_sat=kodesatuan.kode_sat "+ 
                        "inner join jen is on databarang.kdjns=jenis.kdjns "+
                          "where piutang.nota_piutang='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim()+"' and detailpiutang.aturan_pakai<>''",param);
                }    
    ...
    private void ppResepObat2ActionPerformed(java.aw t .ev ent.ActionEvent  e vt) {//GEN-FIRST:event_ppResepObat2ActionPerformed
        if(tabMode.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
            TCari.requestFocus();
        }else if(tbDokter.getSelectedRow()<= -1){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan pilih data..!!");
             ...
             if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim().equals("")){
                Valid.textKosong(TCari,"No.Nota");
                 ...
                 this.setCursor(C ur so r.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Map<String, Object> param = new  HashMap<>();
                param.put("namars",akses.getnamars());
                  param.put("alamatrs",akses .getal amatrs());
                param.put("kotars",akses.getkab upatenrs());
                  param.put("propinsirs",akses.getpropinsirs());
                 param.put("kontakrs",akses.getkontakrs());  
                param.put("emailrs",aks es.getemailrs());
                 p aram.put("logo",Sequel.cariGambar("select setting.logo from setting"));
                if(Sequel.cariInteger(
                        "select count(*) from detailpiutang where nota_piutang=? and aturan_pakai<>''",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim())>0){
                    Valid.MyReportq ry("rptItemResepPiutang3.jasper","report","::[ Aturan Pakai Obat ]::",
                        "select piuta ng.nota_piutang,piutang.tgl_piutang, "+
                        "piutang.no _rkm_medis,piutang.nm_pasien,databarang.nama_brng,"+
                        "detailpiutang. aturan_pakai,detailpiutang.jumlah,kodesatuan.satuan "+
                        "from piutang  inner join detailpiutang on piutang.nota_piutang=detailpiutang.nota_piutang "+
                        "inner join  databarang on detailpiutang.kode_brng=databarang.kode_brng "+
                        "inner jo in kodesatuan on databarang.kode_sat=kodesatuan.kode_sat "+
                        "where piutang.nota_piutang='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim()+"' and detailpiutang.aturan_pakai<>''",param);
                }   
                ...
                this.setCursor(Cursor.getDefaultCursor());  
                                    ...
                                    ResepObat2ActionPerformed 
                                    
                                    
                                    rObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ p
                                    wCount()==0){ 
                                    showMessageDialog(null,"Maaf, 
                                    d ata sudah habis...!!!!"); 
            ...
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan pilih data..!!");
             ...
             if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim().equals("")){
                Valid.textKosong(TCari,"No.Nota");
                 ...
                 kodedokter=Seque l. ca riIsi("select reg_periksa.kd_dokter from reg_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                if(kodedokter.equals("")){ 
                    ...
                    this.setCursor(Cursor.getPr edefinedCursor(Cursor.WAIT_CURSOR));
                      Map<String, Object> param = new HashMap<>();
                     param.put("namars",akses.getnamars());  
                    param.put("alamatrs ",akses.getalamatrs());
                      param.put("kotars",akses.getkabupatenrs());
                    param.put("propinsirs",akses.getpropinsirs());
                    param.put("emailrs",akses.getemailrs());
                    param.put("kont akrs",akses.getkontakrs());
                    param.put("penang gung",Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?",Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim())));
                    param.put("prop insirs",akses.getpropinsirs());
                    param.put("norawat" ,tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                    nomorrm=Sequel.ca riIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                    param.put("norm" ,nomorrm);
                    param.put("ta nggal",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString().trim());
                    ...
                    param.put("noresep","R"+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString()
                        .trim());    
                    param.put("logo",Sequel.cariGambar("select setti ng.logo f rom setting"));
                             
                                    d.MyReport("rptLembarPiutangObat.jasper",param,"::[ Lembar Pem b
    ...
    private void 
                ...
                ionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
            TCari.requestFocus();
        }else if(tbDokter.getSelectedRow()<= -1){
             JOptionPane.showMessageDialog(null,"Maaf, Silahkan pilih data..!!");
        }else{
            if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim( ).equals("")){
                 Valid.textKosong (T Ca ri,"No.Nota");
            }else{ 
                kodedokter=Sequel.cariIsi("select reg_periksa.kd_dokter from reg_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                  if(kodedokter.equals("")){  
                    JOptionPane.showMessageDial og(null,"Dokter Peresep tidak ditemukan, pastikan pasien berasal dari poli/unit..!!!");
                  }else{
                     this.setCursor(Cursor.getPredefinedCursor (Cursor.WAIT_CURSOR)); 
                    Sequel.queryu("dele te from temporary_resep where temp37='"+akses.getalamatip()+"'");
                      try {
                        i=0;
                        ps=koneksi.prepareStatement(
                            "select  databarang.nama_brng,detailpiutang.aturan_pakai,detailpiutang.jumlah,kodesatuan.satuan "+
                            "from det ailpiutang inner join databarang on databarang.kode_brng=detailpiutang.kode_brng "+
                            "inner  join kodesatuan on detailpiutang.kode_sat=kodesatuan.kode_sat  "+
                            "where deta ilpiutang.nota_piutang=? and detailpiutang.aturan_pakai<>''");
                        try { 
                            ps.setSt ring(1,tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                            rs=ps .executeQuery();
                             while(rs.next()){
                                Sequel.menyimpan("temporary_resep","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?
                        ,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,n ew String[]{   
                                    ""+i,rs.getString("nama_brng"),r s.getStri ng("aturan_pakai"),rs.getStr
                                    }); 
                                            i++; 
                                        } 
                                    } catch (Exception e) {
                    ...
                    Map<String, Object> param = new HashMap<>();
                      param.put("namars",aks es.get namars());
                    param.put("alamatrs",akses. getalamatrs());
                      param.put("kotars",akses.getkabupatenrs());
                     param.put("propinsirs",akses.getpropinsir s()); 
                    param.put("emailrs" ,akses.getemailrs());
                      param.put("kontakrs",akses.getkontakrs());
                    param. p ut("penanggung"
                        ,Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?",Se
                        quel.cariIsi("select reg_periksa.kd_pj from re g_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim())));
                     param.put("propinsirs ",akses.getpropinsirs());
                    param.put("norawat",tbDokter.getVal
                            ueAt(tbDokter.getSelectedRow(),0).toString().trim());
                     m em=Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                    param.put("norm",mem);
                    ...
                    namadokter=Sequel.car iIsi("select dokter.nm_dokter from dokter where dokter.kd_dokter=?",kodedokter);
                    param.put("peresep" ,namadokter);
                    param.put("noresep","R" +tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                    Sequel.cariIsi("sele ct sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",kodedokter);
                    param.put("finger","D ikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+"\nDitandatangani secara elektronik oleh "+namadokter+"\nID "+(bar.equals("")?kodedokter:bar)+"\n"+Valid.SetTgl3(tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString().trim()));
                    param.put("logo",Sequel
                            .cariGambar("select setting.logo from setting"));
                                    
                                            
                                             
 
                    Valid.MyReportqry("r ptLembarPiutangObat2.jasper","report","::[ Lem bar Pemberian Obat ]::","select * from temporary_resep where temporary_resep.temp37='"+akses.getalamatip()+"' order by temporary_resep.no",param);
                    this.se t Cursor(Cursor.g
                            etDefaultCursor());
    ...
    }//GEN-LAST:event_ppLembarObat1Actio
                            nPerformed 
    
    private void formWindowOpened(jav a.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        if(koneksiDB.CARICEPAT().equals("aktif")){
            TCari.getDocument().addDocumentListener(new javax.swi ng.eve nt.DocumentListener(){
                @Override
                ...
                @Override 
                 public void remo ve Up date(DocumentEvent e) {
                    if(TCari.getText().length() >2){
                        runBackground(() ->tampil());
                      }  
                } 
    ...
    /**
    * @param args the com m and line arguments
    */  
                                String args[]) {
                                        
                                        invokeLater(() -> {
                                        
                                        ialog = new DlgCariPiutang(new javax.swing.JFrame(), true); 
                                        Listener(new java.awt.event.WindowAdapter() {
                @Override
                public void windowClosing(j ava.awt.event.WindowEvent e) { 
                    System.exi t (0);
                }  
            });
    ...
    private widget.Button BtnBara ng;
    private widget.Button BtnC ari ;  
    private widget.Button BtnKeluar;
    private widget.Button BtnPetugas;
    private widget.Button BtnP rin t;  
    ...
    private widget.InternalFrame intern alFrame1;
    private javax.swing.JPanel jPanel1; 
    private javax.swing.JPopupMenu jPop upMenu1;
    ...
    private widget.Label label 1 8;
    ...
    private void tampil() {
        ...
        if(!NoNota.getText().equals("")){
            nofak=" an d piutang.nota_piutang='" + No Nota.getText()+"' ";
        }         
        ...
        if(!nmptg.getText().equals("")){
            ptg=" and  petugas.nama='"+nmptg.get T ex t()+"' ";
        } 
        ...
        cep t ion
         e)   {
                      ...
                      cicilan=Sequel. c ariIsiAngka("sele c t (sum(bayar_piutang.besar_cicilan)+sum(bayar_piutang.diskon_piutang)+sum(bayar_piutang.tidak_terbayar)) from bayar_piutang where bayar_piutang.no_rawat='"+rs.getString(1)+"' ");
    ...
    public void cariNoT
        ...
        Tgl2.setDate(Taggal);
        runBackground(() ->ta m pil());
    }  
    public void isCek(){  
        MnDetailCicilan.setEnab l ed(akses.getbayar_piutang());
        BtnPrint.setEnabled ( akses.getpiutang_obat());
        ppCetakNota.se t Enabled(akses.getpiutang_obat());
        if(akses.getkod e ().equals("Admin Utama"))
                            { 
                                    ed(true);
                                    ...
                                    own() || executor.isTerminated()) return;
        if (!isDisplayable()) return;
  
        ceksukses = true;  
        setCursor(Cursor.getPredef i nedCur s or(Cursor.WAIT_CURSOR));
        ...
        } catch (RejectedExec utionException ex) {
            ceksukses = fa lse;   
        }
    }
