                ...
                if(NoPermintaan.trim().equals("")){
                    ...
                    if(Sampel.equals("")||akses.getkode().equals("Admin Utama")){
                        if(Sequel.cariInteger("select count(permintaan_pemeriksaan_labpa.noorder) from permintaan_pemeriksaan_labpa where permintaan_pemeriksaan_labpa.stts_bayar='Sudah' and permintaan_pemeriksaan_labpa.noorder=?",NoPermintaan)>0){
                         ...
                         if (Sequel.cariIsi("SELECT EXISTS(SELECT 1 FROM periksa_lab INNER JOIN permintaan_pemeriksaan_labpa ON permintaan_pemeriksaan_labpa.no_rawat = periksa_lab.no_rawat\n"
                                + " AND permintaan_pemeriksaan_labpa.tgl_hasil = periksa_lab.tgl_periksa\n"
                                + " AND permintaan_pemeriksaan_labpa.jam_hasil = periksa_lab.jam  WHERE permintaan_pemeriksaan_labpa.noorder = ? LIMIT 1) AS sudah_periksa", NoPermintaan).equals("1")) {
                            ...
                            int reply = JOptionPane.showConfirmDialog(rootPane, "Permintaan Lab ini sudah dilakukan pengambilan sampel. "
                                    + "Apakah anda yakin ingin menghapus permintaan ini ?", "Konfirmasi", JOptionPane.YES_NO_OPTION);
                            if (reply == JOptionPane.YES_OPTION) {
                                Sequel.meghapus("permintaan_pemeriksaan_labpa", "noorder", NoPermintaan);
                                TeksKosong();
                                tampil3();
                            }
                        }
