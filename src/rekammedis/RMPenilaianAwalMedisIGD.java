...
public final class RMPenilaianAwalMedisIGD extends javax.swing.JDialog {
    ...
    public RMPenilaianAwalMedisIGD(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[] {
                ...
                "Anamnesis", "Hubungan", "Keluhan Utama", "Riwayat Penyakit Sekarang", "Riwayat Penyakit Dahulu",
                "Riwayat Penyakit Keluarga", "Riwayat Penggunakan Obat", "Riwayat Alergi", "Keadaan Umum", "GCS",
                "Kesadaran", "TD(mmHg)", "Nadi(x/menit)", "RR(x/menit)", "Suhu", "SpO2", "BB(Kg)", "TB(cm)", "Kepala",
                "Mata", "Gigi & Mulut", "Leher", "Thoraks", "Abdomen", "Genital & Anus", "Ekstremitas",
                "Ket.Pemeriksaan Fisik", "Ket.Status Lokalis", "EKG", "Radiologi", "Laborat", "Diagnosis/Asesmen",
                "Tatalaksana"
        }) {
            @Override
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Pengkajian Awal Medis IGD ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
        ...
        Kesadaran.setModel(new javax.swing.DefaultComboBoxModel(
                new String[] { "Compos Mentis", "Apatis", "Somnolen", "Sopor", "Koma" }));
        ...
        if (tbObat.getSelectedRow() > -1) {
            if (akses.getkode().equals("Admin Utama")) {
                ...
                if (KdDokter.getText().equals(tbObat.getValueAt(tbObat.getSelectedRow(), 5).toString())) {
                    if (Sequel.cekTanggal48jam(tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString(),
    ...
    private void MnPenilaianMedisActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_MnPenilaianMedisActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            Valid.MyReportqry("rptCetakPenilaianAwalMedisIGD.jasper", "report",
                    "::[ Laporan Pengkajian Awal Medis IGD ]::",
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[] {
                            ...
                            rs.getDate("tgl_lahir"), rs.getString("jk"), rs.getString("kd_dokter"),
                            ...
                            rs.getString("bb"),
                            rs.getString("tb"), rs.getString("kepala"), rs.getString("mata"), rs.getString("gigi"),
                            rs.getString("leher"), rs.getString("thoraks"), rs.getString("abdomen"),
                            rs.getString("genital"),
                            rs.getString("ekstremitas"), rs.getString("ket_fisik"), rs.getString("ket_lokalis"),
                            rs.getString("ekg"), rs.getString("rad"), rs.getString("lab"), rs.getString("diagnosis"),
                            rs.getString("tata")
                    });
                }
            } catch (Exception e) {
    ...
    private void ganti() {
        if (Sequel.mengedittf("penilaian_medis_igd", "no_rawat=?",
                "no_rawat=?,tanggal=?,kd_dokter=?,anamnesis=?,hubungan=?,keluhan_utama=?,rps=?,rpd=?,rpk=?,rpo=?,alergi=?,keadaan=?,gcs=?,kesadaran=?,td=?,nadi=?,rr=?,suhu=?,spo=?,bb=?,tb=?,kepala=?,mata=?,gigi=?,leher=?,thoraks=?,abdomen=?,genital=?,ekstremitas=?,ket_fisik=?,ket_lokalis=?,ekg=?,rad=?,lab=?,diagnosis=?,tata=?",
                37, new String[] {
                        TNoRw.getText(),
                        Valid.SetTgl(TglAsuhan.getSelectedItem() + "") + " "
                        ...
                        Kesadaran.getSelectedItem().toString(), TD.getText(),
                        Nadi.getText(), RR.getText(), Suhu.getText(), SPO.getText(), BB.getText(), TB.getText(),
                        Kepala.getSelectedItem().toString(), Mata.getSelectedItem().toString(),
                        Gigi.getSelectedItem().toString(), Leher.getSelectedItem().toString(),
                        ...
                        KetFisik.getText(), KetLokalis.getText(), EKG.getText(),
    ...
    private void simpan() {
        if (Sequel.menyimpantf("penilaian_medis_igd",
            ...
            tabMode.addRow(new String[] {
                    ...
                    GCS.getText(), Kesadaran.getSelectedItem().toString(),
                    TD.getText(), Nadi.getText(), RR.getText(), Suhu.getText(), SPO.getText(), BB.getText(),
                    TB.getText(), Kepala.getSelectedItem().toString(), Mata.getSelectedItem().toString(),
                    Gigi.getSelectedItem().toString(), Leher.getSelectedItem().toString(),
                    Thoraks.getSelectedItem().toString(), Abdomen.getSelectedItem().toString(),
                    Genital.getSelectedItem().toString(), Ekstremitas.getSelectedItem().toString(), KetFisik.getText(),
                    KetLokalis.getText(), EKG.getText(), Radiologi.getText(), Laborat.getText(),
        ...
        Sequel.mengedit("antripoli", "no_rawat = ?", "status=?, updated_at = now() ", 2,
                new String[] { "1", TNoRw.getText() });
    }

    private void simpanSOAP() {
        String keluhan = "IGD : " + RPS.getText();
        String pemeriksaan = "IGD : " + "Ket Fisik : " + KetFisik.getText() + ", Ket Lokalis : " + KetLokalis.getText()
                + ", TENSI : " + TD.getText() + ", SUHU : " + Suhu.getText() + ", NADI : " + Nadi.getText() + ", GCS : "
                + GCS.getText() + ", TB : " + TB.getText() + ", BB : " + BB.getText() + ", spo : " + SPO.getText()
                + ", RESPIRASI : " + RR.getText();
        if (Sequel.menyimpantf("pemeriksaan_ralan", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Pemeriksaan Ralan",
                21, new String[] {
                        TNoRw.getText(), Valid.SetTgl(TglAsuhan.getSelectedItem() + ""),
                        TglAsuhan.getSelectedItem().toString().substring(11, 19), Suhu.getText(), TD.getText(),
                        Nadi.getText(),
                        RR.getText(), TB.getText(), BB.getText(), SPO.getText(), GCS.getText(),
                        Kesadaran.getSelectedItem().toString().equals("Koma") ? "Coma"
                                : Kesadaran.getSelectedItem().toString(),
                        KeluhanUtama.getText(), pemeriksaan, Alergi.getText(), "-", "-", "-", "-", "-",
                        KdDokter.getText()
                }) == true) {
            emptTeks();
        }
    ...
    private void editSOAP() {
        ...
        String pemeriksaan = "IGD : " + "Ket Fisik : " + KetFisik.getText() + ", Ket Lokalis : " + KetLokalis.getText()
                ...
                + GCS.getText() + ", TB : " + TB.getText() + ", BB : " + BB.getText() + ", spo : " + SPO.getText()
                + ", RESPIRASI : " + RR.getText();
        if (Sequel.mengedittf("pemeriksaan_ralan", "no_rawat=? and tgl_perawatan = ? and jam_rawat = ? and nip = ?",
                "tgl_perawatan = ?,jam_rawat = ?, suhu_tubuh=?,tensi=?,nadi=?,respirasi=?,tinggi=?,berat=?,spo2=?,gcs=?,kesadaran=?,keluhan=?,pemeriksaan=?,alergi=?,lingkar_perut=?,rtl=?,penilaian=?,instruksi=?,evaluasi=?",
                23, new String[] {
                        Valid.SetTgl(TglAsuhan.getSelectedItem() + ""),
                        TglAsuhan.getSelectedItem().toString().substring(11, 19), Suhu.getText(), TD.getText(),
                        Nadi.getText(), RR.getText(), TB.getText(), BB.getText(), SPO.getText(), GCS.getText(),
                        Kesadaran.getSelectedItem().toString().equals("Koma") ? "Coma"
                                : Kesadaran.getSelectedItem().toString(),
                        keluhan, pemeriksaan, Alergi.getText(), "-", "-", "-", "-", "-",
                        tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString(),
                        tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString().substring(0, 10),
                        tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString().substring(11, 19),
                        tbObat.getValueAt(tbObat.getSelectedRow(), 5).toString()
                }) == true) {
            emptTeks();
        }
    ...
    private void hapusSOAP() {
        if (Sequel.queryu2tf(
                "delete from pemeriksaan_ralan where no_rawat=? and tgl_perawatan = ? and jam_rawat = ? and nip = ?", 4,
                new String[] {
                        tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString(),
                        tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString().substring(0, 10),
                        tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString().substring(11, 19),
