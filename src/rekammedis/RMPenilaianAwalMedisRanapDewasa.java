...
public final class RMPenilaianAwalMedisRanapDewasa extends javax.swing.JDialog {
    ...
    public RMPenilaianAwalMedisRanapDewasa(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[] {
                ...
                "Mata", "Gigi & Mulut", "THT", "Thoraks", "Jantung", "Paru", "Abdomen", "Genital & Anus", "Ekstremitas",
                "Kulit", "Ket.Pemeriksaan Fisik", "Ket.Status Lokalis", "Laboratorium", "Radiologi",
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Pengkajian Awal Medis Rawat Inap Umum ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
        ...
        if (TNoRM.getText().trim().equals("")) {
            ...
            if (Sequel.menyimpantf("penilaian_medis_ranap",
                    ...
                    new String[] {
                            ...
                            Thoraks.getSelectedItem().toString(), Jantung.getSelectedItem().toString(),
                            Paru.getSelectedItem().toString(), Abdomen.getSelectedItem().toString(),
    ...
    private void MnPenilaianMedisActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_MnPenilaianMedisActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            Valid.MyReportqry("rptCetakPenilaianAwalMedisRanap.jasper", "report",
                    "::[ Laporan Pengkajian Awal Medis Rawat Inap ]::",
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,if(pasien.jk='L','Laki-Laki','Perempuan') as jk,pasien.tgl_lahir,penilaian_medis_ranap.tanggal,"
                            +
                            "penilaian_medis_ranap.kd_dokter,penilaian_medis_ranap.anamnesis,penilaian_medis_ranap.hubungan,penilaian_medis_ranap.keluhan_utama,penilaian_medis_ranap.rps,penilaian_medis_ranap.rpk,penilaian_medis_ranap.rpd,penilaian_medis_ranap.rpo,penilaian_medis_ranap.alergi,"
                            +
                            ...
                            "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis " +
                            "inner join penilaian_medis_ranap on reg_periksa.no_rawat=penilaian_medis_ranap.no_rawat " +
                            "inner join dokter on penilaian_medis_ranap.kd_dokter=dokter.kd_dokter where penilaian_medis_ranap.no_rawat='"
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[] {
                            ...
                            rs.getString("tht"), rs.getString("thoraks"), rs.getString("jantung"), rs.getString("paru"),
                            rs.getString("abdomen"),
                            rs.getString("genital"), rs.getString("ekstremitas"), rs.getString("kulit"),
                            rs.getString("ket_fisik"), rs.getString("ket_lokalis"), rs.getString("lab"),
                            rs.getString("rad"), rs.getString("penunjang"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("penilaian_medis_ranap", "no_rawat=?",
                "no_rawat=?,tanggal=?,kd_dokter=?,anamnesis=?,hubungan=?,keluhan_utama=?,rps=?,rpk=?,rpd=?,rpo=?,alergi=?,keadaan=?,gcs=?,kesadaran=?,td=?,nadi=?,rr=?,suhu=?,"
                        +
                        "spo=?,bb=?,tb=?,kepala=?,mata=?,gigi=?,tht=?,thoraks=?,jantung=?,paru=?,abdomen=?,genital=?,ekstremitas=?,kulit=?,ket_fisik=?,ket_lokalis=?,lab=?,rad=?,penunjang=?,diagnosis=?,tata=?,edukasi=?",
                41, new String[] {
                        TNoRw.getText(),
                        Valid.SetTgl(TglAsuhan.getSelectedItem() + "") + " "
                                + TglAsuhan.getSelectedItem().toString().substring(11, 19),
                        ...
                        Paru.getSelectedItem().toString(), Abdomen.getSelectedItem().toString(),
                        Genital.getSelectedItem().toString(), Ekstremitas.getSelectedItem().toString(),
                        Kulit.getSelectedItem().toString(), KetFisik.getText(), KetLokalis.getText(), Laborat.getText(),
                        Radiologi.getText(), Penunjang.getText(), Diagnosis.getText(), Tatalaksana.getText(),
                        Edukasi.getText(),
                        tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()
                }) == true) {
            runBackground(() -> tampil());
