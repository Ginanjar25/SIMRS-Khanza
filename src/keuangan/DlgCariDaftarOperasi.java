...
public final class DlgCariDaftarOperasi extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Daftar Tindakan Operasi ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[] {
                            rs.getString("kode_paket"), rs.getString("nm_perawatan"), rs.getString("kategori"),
                            rs.getDouble("total")
                    });
                    iyembuilder.append("{\"KodePaket\":\"").append(rs.getString("kode_paket"))
                            .append("\",\"NamaOperasi\":\"").append(rs.getString("nm_perawatan").replaceAll("\"", ""))
                            .append("\",\"Kategori\":\"").append(rs.getString("kategori")).append("\",\"Total\":\"")
                            .append(rs.getDouble("total")).append("\",\"KodePJ\":\"").append(rs.getString("kd_pj"))
                            .append("\",\"Kelas\":\"").append(rs.getString("kelas")).append("\"},");
                }
            } catch (Exception e) {
                System.out.println("Notifikasi : " + e);
            } finally {
                    ...
                    if (TCari.getText().trim().equals("")) {
                        for (JsonNode list : response) {
                            if (list.path("KodePJ").asText().equals(kd_pj.trim())
                                ...
                                tabMode.addRow(new Object[] {
                                        list.path("KodePaket").asText(), list.path("NamaOperasi").asText(),
                                        list.path("Kategori").asText(), list.path("Total").asDouble()
                                });
                            }
                        }
                    } else {
                        for (JsonNode list : response) {
                            if ((list.path("KodePJ").asText().equals(kd_pj.trim())
                                    || list.path("KodePJ").asText().equals("-"))
                                    && (list.path("KodePaket").asText().toLowerCase()
                                            .contains(TCari.getText().toLowerCase())
                                            || list.path("NamaOperasi").asText().toLowerCase()
                                                    .contains(TCari.getText().toLowerCase()))) {
                                tabMode.addRow(new Object[] {
                                        list.path("KodePaket").asText(), list.path("NamaOperasi").asText(),
                                        list.path("Kategori").asText(), list.path("Total").asDouble()
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[] {
                            ...
                            (rs.getDouble("operator1") + rs.getDouble("operator2") + rs.getDouble("operator3")
                                    ...
                                    + rs.getDouble("bidan") + rs.getDouble("bidan2")
                                    + rs.getDouble("bidan3") + rs.getDouble("perawat_luar") + rs.getDouble("sewa_ok")
                                    + rs.getDouble("akomodasi") + rs.getDouble("bagian_rs") + rs.getDouble("omloop")
                                    + rs.getDouble("omloop2") + rs.getDouble("omloop3") + rs.getDouble("omloop4")
                                    + rs.getDouble("omloop5") + rs.getDouble("sarpras") + rs.getDouble("dokter_pjanak")
                                    + rs.getDouble("dokter_umum"))
                    });
                }
            } catch (Exception e) {
