...
public class BPJSSuratKontrol extends javax.swing.JDialog {
    ...
    private String link = "", requestJson = "", URL = "", user = "", URUTNOREG = "", utc = "",
            JADIKANBOOKINGSURATKONTROLAPIBPJS = "no", kodedokter = "", kodepoli = "", noreg = "";
    ...
    public BPJSSuratKontrol(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[] {
                ...
                "Status PRB",
                "HBA1C", "GDP", "GD2JPP", "eGFR", "TD Sis", "TD Dias", "LDL", "Rerata TD Sis", "Rerata TD Dias",
                "Jantung Koroner",
                ...
                "Bangun Malam", "Keterbatasan Fisik", "Fungsi Paru", "Skor MMRC", "Eksaserbasi 1 Tahun",
                "Mampu Aktivitas",
                "Epileptik 6 Bulan", "Efek Samping OAB", "Hamil Menyusui", "Remisi", "Terapi Rumatan", "Usia",
                "Asam Urat",
                "Remisi SLE", "Hamil"
        }) {
            @Override
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Surat Kontrol VClaim ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
        ...
        StatusPRB.setModel(new javax.swing.DefaultComboBoxModel(
                new String[] { "", "01. Diabetes Melitus", "02. Hipertensi", "03. Asma", "04. Penyakit Jantung",
                        "05. PPOK", "06. Skizofrenia", "07. Stroke", "08. Epilepsi", "09. SLE" }));
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (TanggalSurat.getSelectedItem().toString().trim()
                .equals(TanggalKontrol.getSelectedItem().toString().substring(0, 10))) {
            JOptionPane.showMessageDialog(null,
                    "Tanggal terbit surat kontrol tidak boleh sama dengan tanggal rencana kontrol");
        } else if (!cekSkdpPoli(NoRM.getText(), NoSEP.getText(), TanggalKontrol.getSelectedItem().toString())
                .equals("")) {
            String noSkdpPoli = cekSkdpPoli(NoRM.getText(), NoSEP.getText(),
                    TanggalKontrol.getSelectedItem().toString());
            String data = Sequel.cariIsi(
                    "SELECT CONCAT('Tanggal : ',skdp.tgl_rencana,'\nPoli : ', skdp.nm_poli_bpjs , '\nDokter : ', skdp.nm_dokter_bpjs) FROM bridging_surat_kontrol_bpjs skdp WHERE skdp.no_surat =?",
                    noSkdpPoli);
        ...
        } else if (Sequel.cariInteger("select count(no_surat) from bridging_surat_kontrol_bpjs where no_sep = ?",
                NoSEP.getText()) > 0) {
            ...
            try {
                ...
                if (StatusPRB.getSelectedIndex() == 0) {
                    ...
                    requestJson = "{" +
                            ...
                            "\"TD_Sistolik\": "
                            + (TDSistolik.getText().trim().equals("") ? "null" : TDSistolik.getText()) + "," +
                            "\"TD_Diastolik\": "
                            + (TDDiastolik.getText().trim().equals("") ? "null" : TDDiastolik.getText()) + "," +
                            "\"LDL\": " + (LDL.getText().trim().equals("") ? "null" : LDL.getText()) + "," +
                            "\"Rata_TD_Sistolik\": "
                            + (RerataTDSistolik.getText().trim().equals("") ? "null" : RerataTDSistolik.getText()) + ","
                            +
                            "\"Rata_TD_Diastolik\": "
                            + (RerataTDDiastolik.getText().trim().equals("") ? "null" : RerataTDDiastolik.getText())
                            + "," +
                            "\"JantungKoroner\": "
                            + (JantungKoroner.getSelectedIndex() == 0 ? "null"
                                    : JantungKoroner.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"Stroke\": "
                            + (Stroke.getSelectedIndex() == 0 ? "null"
                                    : Stroke.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"VaskularPerifer\": "
                            + (VaskularPerifer.getSelectedIndex() == 0 ? "null"
                                    : VaskularPerifer.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"Aritmia\": "
                            + (Aritmia.getSelectedIndex() == 0 ? "null"
                                    : Aritmia.getSelectedItem().toString().substring(0, 1))
                            + ", " +
                            "\"AtrialFibrilasi\": "
                            + (AtrialFibrilasi.getSelectedIndex() == 0 ? "null"
                                    : AtrialFibrilasi.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"NadiIstirahat\": "
                            + (RRIstirahat.getText().trim().equals("") ? "null" : RRIstirahat.getText()) + "," +
                            "\"SesakNapas3Bulan\": "
                            + (SesakNapas3Bulan.getSelectedIndex() == 0 ? "null"
                                    : SesakNapas3Bulan.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"NyeriDada3Bulan\": "
                            + (NyeriDada3Bulan.getSelectedIndex() == 0 ? "null"
                                    : NyeriDada3Bulan.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"SesakNapasAktivitas\": "
                            + (SesakNapasAktivitas.getSelectedIndex() == 0 ? "null"
                                    : SesakNapasAktivitas.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"NyeriDadaAktivitas\": "
                            + (NyeriDadaAktivitas.getSelectedIndex() == 0 ? "null"
                                    : NyeriDadaAktivitas.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"Terkontrol\": "
                            + (Terkontrol.getSelectedIndex() == 0 ? "null"
                                    : Terkontrol.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"Gejala2xMinggu\": "
                            + (Gejala2xMinggu.getSelectedIndex() == 0 ? "null"
                                    : Gejala2xMinggu.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"BangunMalam\": "
                            + (BangunMalam.getSelectedIndex() == 0 ? "null"
                                    : BangunMalam.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"KeterbatasanFisik\": "
                            + (KeterbatasanFisik.getSelectedIndex() == 0 ? "null"
                                    : KeterbatasanFisik.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"FungsiParu\": "
                            + (FungsiParu.getText().trim().equals("") ? "null" : FungsiParu.getText()) + "," +
                            "\"SkorMMRC\": " + (SkorMMRC.getText().trim().equals("") ? "null" : SkorMMRC.getText())
                            + "," +
                            "\"Eksaserbasi1Tahun\": "
                            + (Eksaserbasi1Tahun.getSelectedIndex() == 0 ? "null"
                                    : Eksaserbasi1Tahun.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"MampuAktivitas\": "
                            + (MampuAktivitas.getSelectedIndex() == 0 ? "null"
                                    : MampuAktivitas.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"Epileptik6Bulan\": "
                            + (Epileptik6Bulan.getSelectedIndex() == 0 ? "null"
                                    : Epileptik6Bulan.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"EfekSampingOAB\": "
                            + (EfekSampingOAB.getSelectedIndex() == 0 ? "null"
                                    : EfekSampingOAB.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"HamilMenyusui\": "
                            + (HamilMenyusui.getSelectedIndex() == 0 ? "null"
                                    : HamilMenyusui.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            "\"Remisi\": " + (Remisi.getText().trim().equals("") ? "null" : Remisi.getText()) + "," +
                            "\"TerapiRumatan\": "
                            + (TerapiRumatan.getSelectedIndex() == 0 ? "null"
                                    : TerapiRumatan.getSelectedItem().toString().substring(0, 1))
                            + "," +
                            ...
                            + "," +
                            "\"Hamil\": "
                            + (Hamil.getSelectedIndex() == 0 ? "null"
                                    : Hamil.getSelectedItem().toString().substring(0, 1))
                            + " " +
                            "}" +
                            "}" +
                            "}" +
                ...
                if (nameNode.path("code").asText().equals("200")) {
                    ...
                    if (Sequel.menyimpantf("bridging_surat_kontrol_bpjs",
                            "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",
                            "No.Surat", 45, new String[] {
                                    NoSEP.getText(), Valid.SetTgl(TanggalSurat.getSelectedItem() + ""),
                                    response.asText(), Valid.SetTgl(TanggalKontrol.getSelectedItem() + ""),
                                    KdDokter.getText(), NmDokter.getText(), KdPoli.getText(), NmPoli.getText(),
                                    StatusPRB.getSelectedItem().toString().trim(), HBA1C.getText(), GDP.getText(),
                                    GD2JPP.getText(), eGFR.getText(), TDSistolik.getText(), TDDiastolik.getText(),
                                    LDL.getText(), RerataTDSistolik.getText(), RerataTDDiastolik.getText(),
                                    JantungKoroner.getSelectedItem().toString().trim(),
                                    Stroke.getSelectedItem().toString().trim(),
                                    VaskularPerifer.getSelectedItem().toString().trim(),
                                    Aritmia.getSelectedItem().toString().trim(),
                                    AtrialFibrilasi.getSelectedItem().toString().trim(),
                                    RRIstirahat.getText(), SesakNapas3Bulan.getSelectedItem().toString().trim(),
                                    NyeriDada3Bulan.getSelectedItem().toString().trim(),
                                    SesakNapasAktivitas.getSelectedItem().toString().trim(),
                                    NyeriDadaAktivitas.getSelectedItem().toString().trim(),
                                    Terkontrol.getSelectedItem().toString().trim(),
                                    Gejala2xMinggu.getSelectedItem().toString().trim(),
                                    BangunMalam.getSelectedItem().toString().trim(),
                                    KeterbatasanFisik.getSelectedItem().toString().trim(), FungsiParu.getText(),
                                    SkorMMRC.getText(), Eksaserbasi1Tahun.getSelectedItem().toString().trim(),
                                    MampuAktivitas.getSelectedItem().toString().trim(),
                                    Epileptik6Bulan.getSelectedItem().toString().trim(),
                                    EfekSampingOAB.getSelectedItem().toString().trim(),
                                    HamilMenyusui.getSelectedItem().toString().trim(), Remisi.getText(),
                                    TerapiRumatan.getSelectedItem().toString().trim(), Usia.getText(),
                                    AsamUrat.getText(), RemisiSLE.getText(), Hamil.getSelectedItem().toString().trim()
                            }) == true) {
                        emptTeks();
                    ...
                    if (nameNode.path("message").asText().contains("Surat Rujukan ini Masa Berlaku Habis")) {
                        ...
                        // response =
                        // mapper.readTree(api.Decrypt(root.path("response").asText(),utc)).path("noSuratKontrol");
                        Sequel.menyimpan("bridging_surat_kontrol_exp", "?,?,?,?,?,?,?,?,now(),'0000-00-00 00:00:00'", 8,
                                new String[] {
                                        NoSEP.getText(), Valid.SetTgl(TanggalKontrol.getSelectedItem() + ""),
                                        KdDokter.getText(), NmDokter.getText(), KdPoli.getText(), NmPoli.getText(),
                                        keterangan, user = akses.getkode()
                                });
                    }
            ...
            if (R1.isSelected() == true) {
                ...
                Valid.MyReportqry("rptBridgingSuratKontrol.jasper", "report", "::[ Data Surat Kontrol VClaim ]::",
                        "select bridging_sep.no_rawat,bridging_sep.no_sep,bridging_sep.no_kartu,bridging_sep.nomr,bridging_sep.nama_pasien,bridging_sep.tanggal_lahir,"
                                ...
                                +
                                "on bridging_surat_kontrol_bpjs.no_sep=bridging_sep.no_sep where bridging_surat_kontrol_bpjs.tgl_surat between '"
                                + Valid.SetTgl(DTPTanggalSurat1.getSelectedItem() + "") + "' and '"
                                + Valid.SetTgl(DTPTanggalSurat2.getSelectedItem() + "") + "' " +
                                (TCari.getText().trim().equals("") ? ""
                                        : "and (bridging_sep.no_rawat like '%" + TCari.getText().trim()
                                                + "%' or bridging_sep.no_sep like '%" + TCari.getText().trim()
                                                + "%' or bridging_sep.no_kartu like '%" + TCari.getText().trim()
                                                + "%' or " +
                                                "bridging_sep.nomr like '%" + TCari.getText().trim()
                                                + "%' or bridging_sep.nama_pasien like '%" + TCari.getText().trim()
                                                + "%' or bridging_surat_kontrol_bpjs.no_surat like '%"
                                                + TCari.getText().trim() + "%' or " +
                                                "bridging_surat_kontrol_bpjs.nm_poli_bpjs like '%"
                                                + TCari.getText().trim()
                                                + "%' or bridging_surat_kontrol_bpjs.nm_dokter_bpjs like '%"
                                                + TCari.getText().trim() + "%')")
                                +
                ...
                Valid.MyReportqry("rptBridgingSuratKontrol.jasper", "report", "::[ Data Surat Kontrol VClaim ]::",
                        "select bridging_sep.no_rawat,bridging_sep.no_sep,bridging_sep.no_kartu,bridging_sep.nomr,bridging_sep.nama_pasien,bridging_sep.tanggal_lahir,"
                                ...
                                "bridging_surat_kontrol_bpjs.kd_poli_bpjs,bridging_surat_kontrol_bpjs.nm_poli_bpjs from bridging_sep inner join bridging_surat_kontrol_bpjs "
                                +
                                "on bridging_surat_kontrol_bpjs.no_sep=bridging_sep.no_sep where bridging_surat_kontrol_bpjs.tgl_rencana between '"
                                + Valid.SetTgl(DTPTanggalKontrol1.getSelectedItem() + "") + "' and '"
    ...
    private void BtnDokterActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnDokterActionPerformed
        if (KdPoli.getText().equals("") || NmPoli.getText().equals("")) {
            ...
            dokter.SetKontrol(KdPoli.getText(), "2: Rencana Kontrol",
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnEditActionPerformed
        if (NoRawat.getText().trim().equals("") || NoSEP.getText().trim().equals("")) {
            ...
            JOptionPane.showMessageDialog(null,
                ...
                "select count(referensi_mobilejkn_bpjs.nomorreferensi) from referensi_mobilejkn_bpjs where  referensi_mobilejkn_bpjs.status != 'Batal' and referensi_mobilejkn_bpjs.nomorreferensi=?",
                NoSurat.getText()) > 0) {
            JOptionPane.showMessageDialog(null,
                    "No. SKDP Sudah di gunakan untuk antrian Mobile JKN, Silhakan Batalkan dulu");
        } else {
            if (tbObat.getSelectedRow() != -1) {
                try {
                    headers = new HttpHeaders();
                    ...
                    if (StatusPRB.getSelectedIndex() == 0) {
                        ...
                        requestJson = "{" +
                                ...
                                + "," +
                                "\"RemisiSLE\": "
                                + (RemisiSLE.getText().trim().equals("") ? "null" : RemisiSLE.getText()) + "," +
                                "\"Hamil\": "
                                + (Hamil.getSelectedIndex() == 0 ? "null"
                                        : Hamil.getSelectedItem().toString().substring(0, 1))
                                + " " +
                    ...
                    if (nameNode.path("code").asText().equals("200")) {
                        if (Sequel.mengedittf("bridging_surat_kontrol_bpjs", "no_surat=?",
                                "tgl_surat=?,tgl_rencana=?,kd_dokter_bpjs=?,nm_dokter_bpjs=?,kd_poli_bpjs=?,nm_poli_bpjs=?,status_prb=?,HBA1C=?,GDP=?,GD2JPP=?,eGFR=?,TD_Sistolik=?,TD_Diastolik=?,LDL=?,"
                                        +
                                ...
                                44, new String[] {
                                        ...
                                        VaskularPerifer.getSelectedItem().toString().trim(),
                                        Aritmia.getSelectedItem().toString().trim(),
                                        AtrialFibrilasi.getSelectedItem().toString().trim(), RRIstirahat.getText(),
                                        SesakNapas3Bulan.getSelectedItem().toString().trim(),
                                        NyeriDada3Bulan.getSelectedItem().toString().trim(),
                                        SesakNapasAktivitas.getSelectedItem().toString().trim(),
                                        NyeriDadaAktivitas.getSelectedItem().toString().trim(),
                                        Terkontrol.getSelectedItem().toString().trim(),
                                        ...
                                        KeterbatasanFisik.getSelectedItem().toString().trim(), FungsiParu.getText(),
                                        SkorMMRC.getText(),
                                        Eksaserbasi1Tahun.getSelectedItem().toString().trim(),
                                        MampuAktivitas.getSelectedItem().toString().trim(),
                                        Epileptik6Bulan.getSelectedItem().toString().trim(),
                                        EfekSampingOAB.getSelectedItem().toString().trim(),
                                        HamilMenyusui.getSelectedItem().toString().trim(), Remisi.getText(),
                                        TerapiRumatan.getSelectedItem().toString().trim(), Usia.getText(),
                                        AsamUrat.getText(), RemisiSLE.getText(),
                                        Hamil.getSelectedItem().toString().trim(), NoSurat.getText()
                                }) == true) {
                            emptTeks();
                            runBackground(() -> tampil());
                            Sequel.meghapus("bridging_surat_kontrol_exp", "no_sep", NoSEP.getText());
                        ...
                        if (nameNode.path("message").asText().contains("Surat Rujukan ini Masa Berlaku Habis")) {
                            ...
                            // response =
                            // mapper.readTree(api.Decrypt(root.path("response").asText(),utc)).path("noSuratKontrol");
                            Sequel.menyimpan("bridging_surat_kontrol_exp",
                                    "?,?,?,?,?,?,?,?,now(),'0000-00-00 00:00:00'", 8, new String[] {
                                            NoSEP.getText(), Valid.SetTgl(TanggalKontrol.getSelectedItem() + ""),
                                            KdDokter.getText(), NmDokter.getText(), KdPoli.getText(), NmPoli.getText(),
                                            keterangan, user = akses.getkode()
                                    });
                ...
                try {
                    ...
                    while (rs.next()) {
                        tabMode.addRow(new Object[] {
                                rs.getString("no_rawat"), rs.getString("no_sep"), rs.getString("no_kartu"),
                                rs.getString("nomr"), rs.getString("nama_pasien"),
                                ...
                                rs.getString("tgl_rencana"), rs.getString("kd_dokter_bpjs"),
                                rs.getString("nm_dokter_bpjs"), rs.getString("kd_poli_bpjs"),
                                rs.getString("nm_poli_bpjs"),
                                rs.getString("status_prb"), rs.getString("HBA1C"), rs.getString("GDP"),
                                rs.getString("GD2JPP"), rs.getString("eGFR"), rs.getString("TD_Sistolik"),
                                rs.getString("TD_Diastolik"), rs.getString("LDL"),
                                rs.getString("Rata_TD_Sistolik"), rs.getString("Rata_TD_Diastolik"),
                                rs.getString("JantungKoroner"), rs.getString("Stroke"), rs.getString("VaskularPerifer"),
                                rs.getString("Aritmia"), rs.getString("AtrialFibrilasi"), rs.getString("NadiIstirahat"),
                                rs.getString("SesakNapas3Bulan"), rs.getString("NyeriDada3Bulan"),
                                rs.getString("SesakNapasAktivitas"), rs.getString("NyeriDadaAktivitas"),
                                rs.getString("Terkontrol"), rs.getString("Gejala2xMinggu"), rs.getString("BangunMalam"),
                                rs.getString("KeterbatasanFisik"), rs.getString("FungsiParu"), rs.getString("SkorMMRC"),
                                rs.getString("Eksaserbasi1Tahun"), rs.getString("MampuAktivitas"),
                                rs.getString("Epileptik6Bulan"), rs.getString("EfekSampingOAB"),
                                rs.getString("HamilMenyusui"), rs.getString("Remisi"), rs.getString("TerapiRumatan"),
                                rs.getString("Usia"), rs.getString("AsamUrat"), rs.getString("RemisiSLE"),
                                rs.getString("Hamil")
                        });
                    }
                } catch (Exception e) {
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            JK.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 6).toString().replaceAll("P", "PEREMPUAN")
    ...
    private boolean isBooking() {
        ...
        if (status == true) {
            ...
            status = Sequel.menyimpantf2("booking_registrasi", "?,?,?,?,?,?,?,?,?,?,?", "Pasien dan Tanggal", 11,
                    new String[] {
                            Valid.SetTgl(TanggalSurat.getSelectedItem() + ""), "08:00:00", NoRM.getText(),
                            Valid.SetTgl(TanggalKontrol.getSelectedItem() + ""),
                            kodedokter, kodepoli, noreg,
    ...
    private String cekSkdpPoli(String no_rm, String no_sep, String tgl) {
        ...
        if (noSkdp != null && !noSkdp.trim().isEmpty()) {
            String tglSkdpStr = Sequel.cariIsi(
                    "SELECT skdp.tgl_rencana FROM bridging_surat_kontrol_bpjs skdp WHERE skdp.no_surat =?", noSkdp);
